...
?>
    <div data-question="<?= $questionTypeId ?>" class="g_answer_container <?= $isActive ? 'active' : ''?>">
        ...
        <?= Html::a(
            ...
            '#',
            ['onclick' => 'g_add_one(event);']
        )?>
        <table class="table">
            <?php foreach($variants as $variant): ?>
                <?= $this->render(
                    'templates/variant',
                    ['text' => $variant->text, 'hasSuccess' => $variant->id == $answers]
                ) ?>
            <?php endforeach; ?>
        </table>
    </div>
...
$js = $this->render('templates/_js', ['template' => $variantTemplate, 'questionTypeId' => $questionTypeId]);
